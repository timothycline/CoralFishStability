...
for(ff in 1:length(FunctionalGroupList)){
  ...
  if(TRUE){
    uniqueHabSite <- TSregTable_mod %>% pull(HabSite) %>% unique() %>% sort()
  	TSregTable_mod$PropCover <- TSregTable_mod$CoralCover
  	TSregTable_mod$lPropCover <- (TSregTable_mod$CoralCover + 0.01) %>% log()
  ...
  saveRDS(ModelSelectionTable,file=here('Output',paste0('Coral_Gompertz_',FunctionalGroupList[ff],'_InterannualFish.RDS')))
}
